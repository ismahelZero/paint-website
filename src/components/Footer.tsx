    ...
    >
      ...
      <div className="pt-[34px] pb-[70px] text-center lg:mb-[34px] lg:px-[88px] lg:pt-[130px] lg:text-start">
        ...
        >
          <input
            ...
            required
          />
          <button className="bg-black rounded-full px-5" type="submit">
            register now
          </button>
        </form>
      </div>
      ...
      <div className="px-3 lg:grid lg:grid-cols-4 lg:px-0 lg:pt-[96px]">
            ...
            <nav className="flex flex-col gap-3 lg:gap-6">
              {navLinks.map((link, index) => (
                <div key={index} className="flex items-center gap-2">
                  ...
                  {link.badge && (
                    <span className="bg-black text-background rounded-full px-2 py-1 text-xs">
                      {link.badge}
                    </span>
                  )}
                </div>
        ...
        <div className="flex max-w-1/2 flex-col gap-6 lg:col-span-1 lg:me-[112px] lg:max-w-full">
          <div className="space-y-4 rounded-lg max-w-[220px]">
            {socialLinks.map(social => (
              <a
                key={social.name}
                href={social.href}
                className="flex items-center gap-4 bg-[#FFFFFF3D] text-white px-1 py-1 rounded-full hover:bg-purple-900 transition"
              >
                <div className="bg-white text-purple-700 p-2 rounded-full text-sm">
                  {social.icon}
                </div>
